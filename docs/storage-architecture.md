...
S3 object keys are generated by the storage adapter and exposed as:

```text
s3://<bucket>/<object-key>
```

The S3 backend is compatible with AWS S3 and MinIO-style endpoints. It stores
sha256 metadata with uploaded objects and keeps local object storage as the
